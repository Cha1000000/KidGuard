...
- Split-tunnel DNS-фильтр **работает штатно** без lockdown: заблокированные домены → NXDOMAIN,
  остальное резолвится, не-DNS трафик идёт напрямую (ICMP/HTTP проходят).
- **lockdown ON — ДОСТОВЕРНЫЙ тест (2026-07-19, через системный UI, не raw-settings):**
  включён «Постоянная VPN» + «Блокировать соединения без VPN» на Alina. `dumpsys` показал
  НЕПУСТЫЕ «Lockdown filtering rules» (UID-диапазоны фильтруются) — enforcement реально активен.
  Результат: **DNS-фильтр работает (vk.com заблокирован), но весь НЕ-DNS трафик дропается**
  (ping по сырому IP 8.8.8.8/1.1.1.1 не проходит). **Подтверждено: настоящий lockdown ломает
  split-tunnel — ребёнок теряет общий интернет, остаётся только DNS.**
- **Вывод:** «Запрет сайтов» (DNS split-tunnel) НЕСОВМЕСТИМ с системным always-on lockdown.
  Варианты: (A) документировать несовместимость + полагаться на PIN-защиту экрана VPN-настроек
  (веха 6.2 и так не даёт ребёнку выключить VPN → lockdown во многом избыточен); (B) переделать
  DNS-режим на полный туннель с форвардингом всего трафика (tun2socks-подобно, тяжело). Решение
  за Володей. Без lockdown фича работает полностью.
...
## Доработки по фидбэку (2026-07-20)

**UI экрана «Запрет сайтов»:** плашка-предупреждение о системной опции «Блокировать соединения
без VPN» (показывается при активных блокировках); компактные карточки доменов; прокручиваемый
список (`weight(1f)`); выравнивание поля ввода и кнопки; подсказка прижата к низу. Влито.

**Защита от дурака (lockdown):** accessibility ловит системный диалог «Использовать сеть VPN?»
(при попытке включить «Блокировать соединения без VPN») → отменяет (GLOBAL_ACTION_BACK) + показывает
`WarningOverlayManager`. Проверено на Alina (плашка «Не включайте эту опцию», тумблер остаётся
выключен). Влито.

**Момент 1 (VpnService самодостаточен):** `VpnMode` + `VpnModeResolver`; при старте без extras
(системный рестарт) сервис сам читает режим из политики. Влито; обычный путь не сломан.

### Бэклог (найдено при разборе 2026-07-20)

- **Always-on lifecycle (глубже момента 1):** включение системного always-on вызывает `onRevoke`
  → наш `stopSelf` → **VPN не восстанавливается сам** (ни resolve, ни VpnController). Эмуляторный
  always-on нестабилен (`always_on_vpn_app` не закрепился) → **проверять и чинить на реальном
  телефоне Олега.** Не блокер: фича «Запрет сайтов» работает и без always-on.
- **Усиление watchdog:** сейчас accessibility проверяется по `ENABLED_ACCESSIBILITY_SERVICES`
  (включён ли в настройках) — НЕ ловит «дормантный» сервис (после force-stop он в списке, но не
  работает). Идея: сервис ставит отметку «я жив», heartbeat сверяет свежесть. Отложено.
- Программный перезапуск accessibility-сервиса **невозможен** (запрет Android) — только watchdog
  уведомляет родителя.
