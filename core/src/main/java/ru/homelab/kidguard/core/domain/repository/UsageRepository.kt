 ...
 * - [appBonusSpentByPackage] — сколько выданного родителем дополнительного времени приложение уже
 *   израсходовало. Счётчик **параллельный**: те же секунды уже посчитаны в счётчиках выше, и в
 *   «сколько ребёнок пробыл в приложении» он не входит — иначе время задвоилось бы.
 */
interface UsageRepository {
    ...
    /**
     * Израсходованное дополнительное время («пропуск») за день: пакет → секунды. Растёт только
     * пока общий дневной лимит исчерпан и у приложения есть непотраченный бонус — см.
     * `usageTickTargets`. Выданное заранее время благодаря этому не утекает вхолостую.
     */
    fun appBonusSpentByPackage(date: LocalDate): Flow<Map<String, Int>>

    /** Прибавить приложению секунды, списанные с выданного ему дополнительного времени. */
    suspend fun addAppBonusSpentTime(date: LocalDate, packageName: String, seconds: Int)

    /**
     * Обнулить израсходованное дополнительное время приложения за день; `null` — по всем
     * приложениям сразу.
     *
     * Нужно при отмене бонуса родителем: запись бонуса исчезает, а расход остался бы, и следующая
     * выдача погасла бы об него мгновенно (выдали 15 → израсходованы → отменили → выдали снова 15,
     * остаток `15 − 15 = 0`), то есть приложение бы не открылось.
     */
    suspend fun resetAppBonusSpent(date: LocalDate, packageName: String? = null)
