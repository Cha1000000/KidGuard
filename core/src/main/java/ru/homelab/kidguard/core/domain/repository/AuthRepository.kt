...
interface AuthRepository {
    ...
    val parentProfile: Flow<AuthUser?>

    /** Стереть локальную родительскую сессию (токен, профиль). Данные на сервере не трогает. */
    suspend fun clearParentSession()

    /** Удалить учётную запись на сервере. Локальные данные НЕ трогает — это делает вызывающий. */
