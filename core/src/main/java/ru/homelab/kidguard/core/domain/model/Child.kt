...
data class Child(
    ...
    companion object {
        /**
         * Порог «молчания», после которого показываем тревогу.
         *
         * Было 12 часов (решение от 16.07.2026, чтобы не врать каждое утро про выключенный на ночь
         * телефон). Стало 40 минут — после разбора инцидентов 31.08 и 01.09: контроль на телефоне
         * ребёнка останавливают вручную (кнопка «Очистить всё» в списке последних), сам он после
         * этого не поднимается, и единственная защита — чтобы родитель узнал быстро. При 12 часах
         * тревога 01.09 пришла бы к 22:00, то есть через 13 часов после остановки в 08:53.
         *
         * 40 минут = два пропущенных 15-минутных heartbeat плюс запас на сетевой джиттер. Ночную
         * ложь это не возвращает: ночью тревоги подавляются целиком (см. [isQuietHours]).
         */
        val STALE_AFTER: Duration = Duration.ofMinutes(40)
    }
}
